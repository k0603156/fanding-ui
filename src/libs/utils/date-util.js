...
export const getI18nSettings = () => {
  return {
    dayNamesShort: weeks.map(week => t(`fc.datepicker.weeks.${ week }`)),
    dayNames: weeks.map(week => t(`fc.datepicker.weeks.${ week }`)),
    monthNamesShort: months.map(month => t(`fc.datepicker.months.${ month }`)),
    monthNames: months.map((month, index) => t(`fc.datepicker.month${ index + 1 }`)),
    amPm: ['am', 'pm']
  };
};
